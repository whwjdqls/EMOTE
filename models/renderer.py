...
if __name__ == '__main__':
    image_size = 224
    uv_size = 256
    obj_filenametopology_path = '/workspace/audio2mesh/emoca/gdl_apps/EMOCA/demos/image_output/EMOCA_v2_lr_mse_20/0000_0000_0000/EMOCA_v2_lr_mse_20/inputs00/mesh_coarse.obj'
    renderer = SRenderY(image_size, obj_filenametopology_path,
                            uv_size)  # .to(self.device)
    verts = renderer.verts.unsqueeze(0)
    faces = renderer.faces
    attributes = renderer.uvcoords
    
    print(verts.shape)
    print(faces.shape)
    print(attributes.shape)
    rasterizer = renderer.rasterizer
    pixel_values = rasterizer(verts,faces,attributes)
    print(pixel_values)
    print(pixel_values.shape)
    print(max(pixel_values))

    pixel_values = np.array(pixel_values).astype(np.uint8)[0][:3].reshape(224,224,3)
    imageio.imwrite('./pixel_values.png', pixel_values)
