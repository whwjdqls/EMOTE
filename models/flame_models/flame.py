...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        default_shape = torch.zeros(
            [self.batch_size, 300 - config["flame_config"]["shape_params"]],
            dtype=self.dtype,
            requires_grad=False,
        )
        ...
        default_exp = torch.zeros(
            [self.batch_size, 100 - config["flame_config"]["expression_params"]],
            dtype=self.dtype,
            requires_grad=False,
        )
        self.register_parameter(
        ...
        self.use_3D_translation = config["flame_config"]["use_3D_translation"]
        ...
        with open(config["flame_config"]["static_landmark_embedding_path"], "rb") as f:
        ...
        if self.use_face_contour:
            conture_embeddings = np.load(
                config["flame_config"]["dynamic_landmark_embedding_path"],
                allow_pickle=True,
                encoding="latin1",
            )
...
def get_vertices_from_flame(config, FLAME, expression_params, jaw_poses, device):
    ...
    for i in range(sequence_length):
        ...
        if i == 0:
            ...
            vertices = torch.cat([vertices, vertices_.unsqueeze(1)], dim=1) 
            
    return vertices # (BS, T, 15069)