...
def main(args, config) :
    ...
    # Choose epoch for checkpoint
    # replace this with actual ckpt path
    FLINT_epoch = find_ckpt_epoch(args, 'FLINT')
    FLINT_ckpt = f'{args.save_dir}/FLINT/{FLINT_epoch}.pt'
    
    # Load model
    model = EMOTE(config, FLINT_config, FLINT_ckpt)
    EMOTE_epoch = find_ckpt_epoch(args, 'EMOTE')
    EMOTE_ckpt_path = f'{args.save_dir}/EMOTE/{EMOTE_epoch}.pt'
    EMOTE_ckpt = modify_EMOTE_ckpt(EMOTE_ckpt_path)
    model.load_state_dict(EMOTE_ckpt)
    
    wavdata, sampling_rate = read_audio(audio_path)
    sample = process_audio(wavdata, sampling_rate, 30)
    dl = torch.utils.data.DataLoader(TestDataset(sample), batch_size=64, shuffle=False, num_workers=0, collate_fn=robust_collate)
    input_style = torch.eye(43)[[3,9,31]]
    input_style = torch.sum(input_style, dim=0).unsqueeze(0).repeat(3,1)
    inputs = []
    outputs = []
    ...
    input_dl = torch.utils.data.DataLoader(inputs, batch_size = 3, shuffle=False)
    for bi, batch in enumerate(input_dl) :
        batch = batch.type(torch.float32)
        output = model(batch, input_style).squeeze(0)
    ...
    np.save('01_gday.npy', outputs)
    


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--EMOTE_config', type=str, required=True, default='/workspace/audio2mesh/EMOTE/configs/EMOTE/EMOTE_inferno.json')
    parser.add_argument('--checkpoint', type=str, default=None)
    parser.add_argument('--num_ckpt', type=int)
    parser.add_argument('--best_ckpt', action='store_true')
    parser.add_argument('--last_ckpt', action='store_true')
    parser.add_argument('--save_dir', type=str, default='/workspace/audio2mesh/EMOTE/checkpoints')
